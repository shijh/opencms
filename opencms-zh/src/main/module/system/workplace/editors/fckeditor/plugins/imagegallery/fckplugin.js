...
dialogCommand.prototype.Execute = function() {
	var command=new FCKDialogCommand(
			'OcmsImageGallery',
			'<%= dialog.key("GUI_IMAGE_GALLERY_TITLE_0") %>',
...
var opencmsImageGalleryItem = new FCKToolbarButton('OcmsImageGallery', '<%= dialog.key("GUI_IMAGE_GALLERY_TITLE_0") %>', '<%= dialog.key("GUI_IMAGE_GALLERY_TITLE_0") %>', null, false, true);
...
FCK.ContextMenu.RegisterListener(
    {
        AddItems : function( menu, tag, tagName )
        {
            ...
            menu.AddItem( "Paste"    , FCKLang.Paste    , 9, FCKCommands.GetCommand('Paste').GetState() == FCK_TRISTATE_DISABLED ) ;
                        ...
                        menu.AddItem("OcmsImageGallery", "<%= dialog.key("GUI_IMAGE_GALLERY_TITLE_0") %>", 37);
                }
        }
    }
); 
