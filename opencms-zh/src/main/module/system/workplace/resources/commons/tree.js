...
//moditied by Shi Jinghai, huaruhai@hotmail.com    2014-4-16
...
function aC(name, title, type, folder, id, parentId, state, grey) {
    ...
    if (!oldNode) {
        ...
        if (!theParent) {
            ...
            tree.root      = new nodeObject(name, title, type, folder, id, null, state, grey, false);
            tree.nodes[id] = tree.root;
        } else {
            // this is a regular subnode
            tree.nodes[id] = new nodeObject(name, title, type, folder, id, parentId, state, grey, false);
            if (!theParent.childs) {
                theParent.childs = [];
            }
