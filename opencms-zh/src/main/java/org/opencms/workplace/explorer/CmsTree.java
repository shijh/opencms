...
public class CmsTree extends CmsWorkplace {
	...
	/**
	 * Indicates if only folders or files and folders should be included in the
	 * tree.
	 */
	...
	/**
	 * Indicates project awareness, ie. if resources outside of the current
	 * project should be displayed as normal.
	 */
	...
	/**
	 * Flag to indicate if the site selector should be shown in popup tree
	 * window.
	 */
	...
	public static String initTree(CmsObject cms, String encoding, String skinUri) {
		...
		CmsMessages messages = OpenCms.getWorkplaceManager()
				.getMessages(locale);
		...
		for (int i = 0; i < allResTypes.size(); i++) {
			...
			CmsExplorerTypeSettings settings = OpenCms.getWorkplaceManager()
					.getExplorerTypeSetting(curTypeName);
			if (settings == null) {
				if (LOG.isWarnEnabled()) {
					LOG
							.warn(Messages.get().getBundle().key(
									Messages.LOG_MISSING_SETTINGS_ENTRY_1,
									curTypeName));
				}
				settings = OpenCms.getWorkplaceManager()
	...
	/**
	 * Determines the root folder of the current tree dependent on users setting
	 * of explorer view restriction.
	 * <p>
	 * 
	...
	public String getRootFolder() {
		...
		if (m_rootFolder == null) {
			...
			try {
				getCms()
						.readFolder(folder, CmsResourceFilter.IGNORE_EXPIRATION);
	...
	public String getSiteSelector(String htmlAttributes) {
		...
		if (preSelection == null) {
			if ("".equals(getCms().getRequestContext().getSiteRoot())) {
				// we are in the root site, getCurrentSite(CmsObject) includes
				// NOT the root site
				preSelection = "";
			} else {
				// get the site root of the current site
				preSelection = OpenCms.getSiteManager()
						.getCurrentSite(getCms()).getSiteRoot();
			}
			// set the tree site to avoid discrepancies between selector and
			// tree
		...
		if (TYPE_PAGELINK.equals(getTreeType())) {
			// in wysiwyg editor link dialog, don't show root site, but show
			// site URLs
			includeRootSite = false;
			showSiteUrls = true;
		}
		List sites = OpenCms.getSiteManager().getAvailableSites(getCms(),
				includeRootSite);
	...
	public String getTree() {
		...
		if (getTargetFolder() != null) {
			// check if there is more than one folder to update (e.g. move
			// operation)
		...
		try {
			...
			while (targets.hasNext()) {
				...
				if (getSettings().getTreeSite(getTreeType()) != null) {
					...
					getCms().getRequestContext().setSiteRoot(
							getSettings().getTreeSite(getTreeType()));
					try {
				...
				if ((startFolder == null)
						|| (!currentTargetFolder.startsWith(startFolder))) {
					...
					// valid start folder given, load all folders between start
					// and current folder
					try {
						if (includeFiles()) {
							resources.addAll(getCms().getResourcesInFolder(
						...
						StringTokenizer tok = new StringTokenizer(
								currentTargetFolder.substring(startFolder
										.length()), "/");
			...
			// read the list of project resource to select which resource is
			// "inside" or "outside"
			List projectResources = new ArrayList();
			if (isProjectAware()) {
				try {
					projectResources = getCms().readProjectResources(
			...
			String title = null;
			CmsProperty titleProperty = null;
			...
			if (treelabel == 1) {
				...
				while (i.hasNext()) {
					...
					if (isProjectAware()) {
						grey = !CmsProject.isInsideProject(projectResources,
								resource);
					}
					if (!grey) {
						try {
					...
					try {
						titleProperty = getCms().readPropertyObject(
								getCms().getRequestContext().removeSiteRoot(
					...
					result.append(getNode(resource.getRootPath(), resource
							.getName(), title, resource.getTypeId(), resource
							.isFolder(), resource.getState(), grey));
				}
			} else {
				// now output all the tree nodes
				Iterator i = resources.iterator();
			...
			/**
			 * Iterator i = resources.iterator(); while (i.hasNext()) {
			 * CmsResource resource = (CmsResource)i.next(); boolean grey =
			 * false; if (isProjectAware()) { grey =
			 * !CmsProject.isInsideProject(projectResources, resource); } if
			 * (!grey) { try {
			 * OpenCms.getResourceManager().getResourceType(resource
			 * .getTypeId()); } catch (CmsLoaderException e) { // if resource
			 * type is not found grey = true; } }
			 * 
			 * result.append(getNode( resource.getRootPath(),
			 * resource.getName(), resource.getTypeId(), resource.isFolder(),
			 * resource.getState(), grey)); }
			 */
			if (includeFiles()) {
				result.append("parent.setIncludeFiles(true);\n");
			}
			result.append("parent.setProjectAware(").append(isProjectAware())
					.append(");\n");
			if (getTreeType() != null) {
			...
			if (folder != null) {
				if (newTree()) {
					// new tree
					result
							.append("parent.showTree(parent.tree_display.document, \"");
					result.append(folder.getRootPath().hashCode());
					result.append("\");\n");
				} else {
					// update the current tree with the children of the selected
					// node
					if (resources.size() == 0) {
					...
					result
							.append("parent.showLoadedNodes(parent.tree_display.document,\"");
					result.append(folder.getRootPath().hashCode());
					result.append("\");\n");
				}
			}
	...
	/**
	 * Returns the type of this tree (e.g. "copy", "project" etc.), if null this
	 * is the default explorer version.
	 * <p>
	 * 
	...
	/**
	 * Indicates if the site selector should be shown depending on the tree
	 * type, initial settings and the count of accessible sites.
	 * <p>
	 * 
	...
	protected void initWorkplaceRequestValues(CmsWorkplaceSettings settings,
		...
		setIncludeFiles(Boolean.valueOf(
				request.getParameter(PARAM_INCLUDEFILES)).booleanValue());
		setProjectAware(Boolean.valueOf(
				request.getParameter(PARAM_PROJECTAWARE)).booleanValue());
		boolean rootloaded = Boolean.valueOf(
				request.getParameter(PARAM_ROOTLOADED)).booleanValue();
		...
		if (getTreeType() == null) {
			...
			// get the current tree resource from the settings for a special
			// tree type
		...
		if (rootFolder.equals(resource) && !rootFolder.equals(currentResource)
				&& (lastknown == null) && !rootloaded) {
			// direct load of a new tree with subtree (e.g. when returning from
			// an editor)
	...
	/**
	 * Determines if the site selector frame should be shown depending on the
	 * tree type or the value of a request parameter.
	 * <p>
	 * 
	 ...
	 * 
	 * @param request
	 *            the HttpServletRequest to check
	 */
	private void computeSiteSelector(HttpServletRequest request) {
		...
		boolean showFromRequest = Boolean.valueOf(
				request.getParameter(PARAM_SHOWSITESELECTOR)).booleanValue();
		if (selectorForType || showFromRequest) {
			// get all available sites
			int siteCount = OpenCms.getSiteManager().getAvailableSites(
					getCms(), true).size();
	 ...
	 */
	private String getNode(String path, String title, int type, boolean folder,
			CmsResourceState state, boolean grey) {
	...
	private String getRootNode() {
		...
		try {
			...
			CmsProperty titleProperty = getCms().readPropertyObject(folder,
					CmsPropertyDefinition.PROPERTY_TITLE, false);
			...
			return getNode(resource.getRootPath(), title, title, resource
					.getTypeId(), true, resource.getState(), false);
			// return getNode(resource.getRootPath(), title,
			// resource.getTypeId(), true, resource.getState(), false);
		} catch (CmsException e) {
	...
	/**
	 * Calculates the prefix that has to be added when selecting a resource in a
	 * popup tree window.
	 * <p>
	 * 
	 * This is needed for the link dialog in editors as well as the copy, move
	 * and link popup dialogs for resources in the VFS.
	 * <p>
	 * 
	 * @param prefix
	 *            the current prefix of the resource
	 * @param storedSiteRoot
	 *            the site root in which the workplace (not the tree!) is
	...
	private String getSitePrefix(String prefix, String storedSiteRoot) {
		...
		if (TYPE_PAGELINK.equals(getTreeType())) {
			...
			if (!storedSiteRoot.equals(prefix)) {
				// stored site is not selected site, create complete URL as
				// prefix
				CmsSite site = OpenCms.getSiteManager().getSiteForSiteRoot(
						prefix);
				prefix = getCms().getRequestContext().removeSiteRoot(prefix);
				prefix = site.getUrl()
						+ OpenCms.getSystemInfo().getOpenCmsContext() + prefix;
			} else {
			...
			// in vfs copy|move|link or vfs widget mode, don't add the prefix
			// for the current workplace site
			if (storedSiteRoot.equals(prefix)) {
				prefix = "";
			}
	...
	/**
	 * Returns the name of the start folder (or "last known" folder) to be
	 * loaded.
	 * <p>
	 * 
	 * @return the name of the start folder (or "last known" folder) to be
	 *         loaded
	 */
	...
	/**
	 * Sets the value to indicate if only folders or files and folders should be
	 * included in the tree.
	 * <p>
	 * 
	 * @param includeFiles
	 *            if true if files and folders should be included in the tree
	 */
	 ...
	 * 
	 * @param newTree
	 *            if true, a complete tree must be loaded
	 */
	...
	/**
	 * Sets if the site selector should be shown depending on the tree type and
	 * the count of accessible sites.
	 * <p>
	 * 
	 * @param showSiteSelector
	 ...
	 * 
	 * @param startFolder
	 *            the name of the start folder (or "last known" folder) to be
	 *            loaded
	 */
	 ...
	 * @return the output for a tree node
	 * 
	 * @author Shi Yusen, shiys@langhua.cn 2010-10-13
	 */
