...
public class CmsUserSettings {
    ...
    public void init(CmsUser user) {
        ...
        // explorer tree label. added by Shi Yusen, shiys@langhua.cn    2010-10-12    
        try {
            m_explorerTreeLabel = ((Integer)m_user.getAdditionalInfo(PREFERENCES
                + CmsWorkplaceConfiguration.N_EXPLORERGENERALOPTIONS
                + CmsWorkplaceConfiguration.N_TREELABEL)).intValue();
        } catch (Throwable t) {
            m_explorerTreeLabel = OpenCms.getWorkplaceManager().getDefaultUserSettings().getExplorerTreeLabel();
        }
        // explorer tree Sort. added by Shi Yusen, shiys@langhua.cn    2010-10-12    
        try {
            m_explorerTreeSort = ((Integer)m_user.getAdditionalInfo(PREFERENCES
                + CmsWorkplaceConfiguration.N_EXPLORERGENERALOPTIONS
                + CmsWorkplaceConfiguration.N_TREESORT)).intValue();
        } catch (Throwable t) {
            m_explorerTreeSort = OpenCms.getWorkplaceManager().getDefaultUserSettings().getExplorerTreeSort();
        }
        // explorer settings
        try {
    ...
    public void save(CmsObject cms) throws CmsException {
        ...
        if (getExplorerTreeLabel() != OpenCms.getWorkplaceManager().getDefaultUserSettings().getExplorerTreeLabel()) {
            m_user.setAdditionalInfo(PREFERENCES
                + CmsWorkplaceConfiguration.N_EXPLORERGENERALOPTIONS
                + CmsWorkplaceConfiguration.N_TREELABEL, new Integer(getExplorerTreeLabel()));
        } else if (cms != null) {
            m_user.deleteAdditionalInfo(PREFERENCES
                + CmsWorkplaceConfiguration.N_EXPLORERGENERALOPTIONS
                + CmsWorkplaceConfiguration.N_TREELABEL);
        }
        //explorer tree sort. added by Shi Yusen, shiys@langhua.cn  2010-10-12
        if (getExplorerTreeSort() != OpenCms.getWorkplaceManager().getDefaultUserSettings().getExplorerTreeSort()) {
            m_user.setAdditionalInfo(PREFERENCES
                + CmsWorkplaceConfiguration.N_EXPLORERGENERALOPTIONS
                + CmsWorkplaceConfiguration.N_TREESORT, new Integer(getExplorerTreeSort()));
        } else if (cms != null) {
            m_user.deleteAdditionalInfo(PREFERENCES
                + CmsWorkplaceConfiguration.N_EXPLORERGENERALOPTIONS
                + CmsWorkplaceConfiguration.N_TREESORT);
        }
        // explorer settings
    ...
    //add by Shi Yusen, shiys@langhua.cn 2010-10-12
    /**
     * Returns the explorer tree label of the user.<p>
     * 
     * @return the tree label of the user
     * 
     */
    public int getExplorerTreeLabel() {

        return m_explorerTreeLabel;
    }

    /**
     * Sets the explorer tree label of the user.<p>
     * 
     * @param style the explorer tree label of the user
     * 
     */
    public void setExplorerTreeLabel(int label) {

        m_explorerTreeLabel = label;
    }
    /**
     * Returns the explorer tree Sort of the user.<p>
     * 
     * @return the tree Sort of the user
     * 
     */
    public int getExplorerTreeSort() {

        return m_explorerTreeSort;
    }

    /**
     * Sets the explorer tree Sort of the user.<p>
     * 
     * @param style the explorer tree Sort of the user
     * 
     */
    public void setExplorerTreeSort(int Sort) {

        m_explorerTreeSort = Sort;
    }
    //end.
}