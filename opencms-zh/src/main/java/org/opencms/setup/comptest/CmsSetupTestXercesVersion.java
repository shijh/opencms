...
public class CmsSetupTestXercesVersion implements I_CmsSetupTest {
    ...
    /** The test name. */
    public static final String TEST_NAME = "Xerces版本";
    ...
    public CmsSetupTestResult execute(CmsSetupBean setupBean) throws Exception {
        ...
        switch (xercesVersion) {
            case 2:
                ...
                testResult.setHelp("OpenCms需要Xerces版本2才能运行。通常它是servlet环境的一部分。");
                testResult.setGreen();
                break;
            case 1:
                testResult.setResult(versionStr);
                testResult.setRed();
                testResult.setInfo("OpenCms需要Xerces版本2才能运行，你的Xerces版本是1。"
                    + "通常Xerces 2应该作为servlet环境的一部分被缺省安装了。");
            ...
            default:
                ...
                testResult.setResult(versionStr);
                testResult.setRed();
                testResult.setInfo("OpenCms需要Xerces版本2才能运行，"
                    + "通常Xerces 2应该作为servlet环境的一部分被缺省安装了。");
                testResult.setHelp(testResult.getInfo());
        }
        return testResult;
